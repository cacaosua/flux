...
void main() {
  ...
  group('isCompatibleWithR13n', () {
    test('returns true when the package:flux_r13n version is compatible', () {
    ...
    test('returns false when the package:flux_r13n version is incompatible',
        () {
  ...
  group('ensureRuntimeCompatibility', () {
    ...
    test(
        ...
        'not contain a package:flux_r13n dependency', () {
      const expected =
...
''',
    ...
    test('throws when the version of package:flux_r13n is incompatible', () {
      const incompatibleVersion = '^99.99.99';
      const expected =
          '''The current version of "brick:flux_r13n" requires "package:flux_r13n" $compatibleR13nVersion.\nBecause the current version of "package:flux_r13n" is $incompatibleVersion, version solving failed.''';
      File(path.join(tempDir.path, 'pubspec.yaml')).writeAsStringSync(
        '''
name: example
version: 0.1.0+1
...
environment:
  sdk: ">=2.17.0 <3.0.0"
dependencies:
  flux_r13n: "$compatibleR13nVersion"
''',
      );
