...
class R13nConfiguration {
  ...
  static const _fileName = 'flux_r13n.yaml';
...
class YamlNotFoundException extends R13nException {
  YamlNotFoundException()
      : super(
          message: 'No flux_r13n.yaml found.',
        );
}
...
/// {@template flux_r13n_compatibility_exception}
/// An exception thrown when the current version of the flux_r13n brick
/// is incompatible with the flux_r13n runtime being used.
/// {@endtemplate}
class R13nCompatibilityException extends R13nException {
  /// {@macro flux_r13n_compatibility_exception}
...
/// The version range of package:flux_r13n
/// supported by the current version of the flux_r13n brick.
const compatibleR13nVersion = '>=0.1.0-dev.1 <0.1.0-dev.3';

/// Whether current version of the flux_r13n brick is compatible
/// with the provided [version] of package:flux_r13n.
...
/// Ensures that the current version of `brick:flux_r13n` is compatible
/// with the version of `package:flux_r13n` used in the [cwd].
void ensureRuntimeCompatibility(Directory cwd) {
  ...
  if (dependencyEntry.isEmpty) {
    throw const R13nCompatibilityException(
      message:
          'Expected to find a dependency on "flux_r13n" in the pubspec.yaml',
    );
  }
  ...
  if (dependency is HostedDependency) {
    if (!isCompatibleWithR13n(dependency.version)) {
      throw R13nCompatibilityException(
        message:
            '''The current version of "brick:flux_r13n" requires "package:flux_r13n" $compatibleR13nVersion.\nBecause the current version of "package:flux_r13n" is ${dependency.version}, version solving failed.''',
      );
    }
  }
}
